...
$('#all-files-table').on('change', function(){
  $('#delete-button').toggle($('input:checkbox:checked').length > 0);
});

$('#delete-button').on('click', function() {
  var username = '';
  var checkedValues = $("input:checkbox:checked", "#all-files-table-body").map(function() {
    return $(this).val();
  }).get();
  var checkedDataVal = $("input:checkbox:checked", "#all-files-table-body").map(function() {
...
$('#all-files-table').on('click', '.delete-link', function(e) {
  e.preventDefault();
